...
class userDAO
{
    ...
    public function salvar($user)
    {
        ...
        try {
            if ($user->getIdUsuario() != "") {
                $statement = $pdo->prepare("UPDATE usuario SET Nome=:Nome, Login=:Login, Senha=:Senha, TipoUsuario_idTipoUsuario=:TipoUsuario_idTipoUsuario WHERE idUsuario = :id;");
                $statement->bindValue(":id", $user->getIdUsuario());
            } else {
    ...
    public function atualizar($user)
    {
        global $pdo;
        try {
            $statement = $pdo->prepare("SELECT idUsuario, login, senha, nome, TipoUsuario_idTipoUsuario FROM usuario WHERE idUsuario = :id");
            $statement->bindValue(":id", $user->getIdUsuario());
            if ($statement->execute()) {
                ...
                $user->setTipoUsuario($rs->TipoUsuario_idTipoUsuario);
                return $user;
            } else {
                throw new PDOException("<script> alert('Não foi possível executar a declaração SQL !'); </script>");
            }
   ...
   /* public function recuperaSenha($login, $email){
        ...
        $sql = "SELECT count(*) as qtd, idUsuario FROM usuario WHERE login=:login AND email =:email ";
        ...
        if($encontrou->qtd > 0){
            $statement = $pdo->prepare("UPDATE usuario SET resetar=:resetar, senha=:senha WHERE iduser =:iduser;");
    ...
    public function tabelapaginada()
    {
        ...
        $sql = "SELECT iduser, login, senha, nome, email, resetar, perfil FROM usuario LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
        $statement = $pdo->prepare($sql);
        $statement->execute();
     ...
     <thead>
       <tr style='text-transform: uppercase;' class='active'>
        ...
        <th style='text-align: center; font-weight: bolder;'>Tipo Usuario</th>";
